...
- TypeScript 7.0 rewrote the compiler in Go and no longer ships `tsserver.js`. The new LSP is `tsc --lsp --stdio`. In `nvim-lspconfig`, use server name `tsc` instead of `ts_ls`. The `tsc` binary is a native binary (not mason-managed), so filter it (and `rust_analyzer`) out of mason's `ensure_installed`. The `tsc` server config is already bundled in nvim-lspconfig; `tsgo` is a deprecated alias for `tsc`.
